...
{
  fprintf(file, "[%4ld,%4ld,%4ld |%4ld,%4ld,%4ld] -> r = %4ld, s = %4ld, p1 = %4ld", k_[0],k_[1],k_[2],l_[0],l_[1],l_[2],r_, s_, p1_);
}
...
bool Space::conditionC(void)
{
  ...
  if (good_col_or_row == -1) // condition has not yet been checked
	...
	if(gcdA(0,c,1,c) == 1 && gcdA(0,c,2,c) == 1 && gcdA(1,c,2,c) == 1)
	  {
	    good_col_or_row = c;// 0/1/2 = col 1/2/3 satisfies condition C
	    break;
	  }
      }
      for(int r = 0; r < 3; ++r){
	if(gcdA(r,0,r,1) == 1 && gcdA(r,0,r,2) == 1 && gcdA(r,1,r,2) == 1)
	  {
	    good_col_or_row = r + 3;// 3/4/5 = row 1/2/3 satisfies condition C
	    break;
	  }
      }
    }
...
void Space::compute_s2_col(int j)
{
  ...
  long long q = 
    ...
    - square(l_[j]-l_[jp1]);
  long long d = 16*3*(long)(r_)*(k_[0]-l_[j])*(k_[1]-l_[j])*(k_[2]-l_[j]);  // check sign of r here!
  s2_.assign(q-2,d);
  //printf("q = %lld, r = %ld, d = %lld\n", q, (long)(r_), d); // check sign of r here!
...
void Space::compute_s2_row(int j)
  {
    ...
    long q = 
      ...
      - square(k_[j]-k_[jp1]);
    long d = 16*3*(long)(r_)*(k_[j]-l_[0])*(k_[j]-l_[1])*(k_[j]-l_[2]);// check sign of r!
    s2_.assign(q-2,d);
    // printf("q = %ld, d = %ld\n", q, d);
    // printf("=> (q-2)/d = %ld/%ld\n",s2_.numerator(),s2_.denominator());
    for(int i = 0; i < 3; ++i)
